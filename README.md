...
| ----      | ----------  | ----------  |
| -b        | Board      | Specifies the device code(s) to build. For example, `-b s905x3` builds the device with code s905x3. Multiple device codes can be connected using underscores, such as `-b s905x3_s905d`. Special values: all compiles all devices; `first55` compiles the first 55 devices in the library; `from55` compiles from the 56th device to the end; `last20` compiles the last 20 devices. The full list of device codes can be found in the `BOARD` entries of [model_database.conf](build-armbian/armbian-files/common-files/etc/model_database.conf). Default: `all` |
