...
int mt7663s_mcu_init(struct mt7615_dev *dev)
{
	...
	struct mt76_mcu_ops *transport_ops = NULL;
	struct mt7615_mcu_ops *mcu_ops;
	bool w103d = of_machine_is_compatible("zte,w103d");
	int ret;
	...
	if (w103d) {
		transport_ops = devm_kmemdup(dev->mt76.dev, &mt7663s_mcu_ops,
					    sizeof(*transport_ops), GFP_KERNEL);
		if (!transport_ops)
			return -ENOMEM;

		transport_ops->headroom =
			mt7663s_w103d_fw_headroom(&dev->mt76);
		transport_ops->mcu_skb_send_msg =
			mt7663s_w103d_mcu_send_message;
		transport_ops->mcu_send_msg = mt7663s_w103d_mcu_send_msg;
		transport_ops->mcu_rr = mt7663s_w103d_reg_rr;
		transport_ops->mcu_wr = mt7663s_w103d_reg_wr;
		dev->mt76.mcu_ops = transport_ops;
	}
	...
	if (ret)
		return ret;
	if (w103d)
		transport_ops->headroom = sizeof(struct mt7615_mcu_txd);
	...
	if (w103d)
		mt7663s_w103d_install_legacy_ops(mcu_ops);
	dev->mcu_ops = mcu_ops;
