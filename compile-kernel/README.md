...
| -g        | config      | Sets whether to download the specified tag's [config-*](https://github.com/ophub/kernel/tree/main/kernel-config/release) files from the kernel repository to the local [compile-kernel/tools/config](tools/config) directory. If the local configuration file corresponding to the kernel version already exists and downloading is not set, it will be ignored. Options: `[ stable / rk3588 / rk35xx / h6 ]`. Default value: `stable` |
