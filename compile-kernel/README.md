...
| -g        | config      | Sets whether to download the specified tag's [configuration](https://github.com/ophub/kernel/tree/main/kernel-config/release) files from the kernel repository to the local `compile-kernel/tools/config` directory. If the local configuration file corresponding to the kernel version already exists and downloading is not set, it will be ignored. Options: `[ stable / rk3588 / rk35xx / h6 ]`. Default value: `stable` |
...
💡 Tip: We recommend using the kernel source code from repositories such as [linux-6.1.y](https://github.com/unifreq/linux-6.1.y), [linux-5.15.y](https://github.com/unifreq/linux-5.15.y), [linux-5.10.y](https://github.com/unifreq/linux-5.10.y) of `unifreq` for compilation, who added drivers and patches for related boxes. It is recommended to use the template in [tools/config](tools/config), which has been pre-configured according to the related boxes and can be customized based on this.
