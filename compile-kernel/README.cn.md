...
| -g     | config      | 设置从内核仓库下载指定标签的[配置](https://github.com/ophub/kernel/tree/main/kernel-config/release)文件到本地 `compile-kernel/tools/config` 目录。当本地已经有内核版本对应的配置文件并且未设置下载时将忽略。可选项：[ `stable / rk3588 / rk35xx / h6` ]。默认值：`stable` |
