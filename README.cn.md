...
2. 全新编译：在 [Action](https://github.com/ophub/amlogic-s9xxx-armbian/actions) 页面里选择 ***`Build armbian`*** ，在 RELEASE 里可选择 Ubuntu 系列：`bionic` / `focal` / `hirsute` ，或者 Debian 系列：`bullseye` / `buster` / `stretch` ，在 BOARD 里可选 `lepotato` / `odroidn2` 等，点击 ***`Run workflow`*** 按钮即可编译。
